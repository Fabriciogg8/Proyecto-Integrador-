...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${GET_CURRENT_PRODUCT}?page=${currentPage}&order=random`,
        )
        const data = await response.json()
  ...
  return (
    <>
      ...
      <Container>
        {selectedCategories.length > 0 && (
          <button className='botonFiltro text-light' onClick={clearFilters}>
            Limpiar Filtros
          </button>
        )}

        <p className='leyendaFiltros text-light'>
          Productos filtrados: {filteredProductsCount} / Total:{' '}
          {totalProductsCount}
        </p>
        <p className='leyendaFiltros text-light'>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <p className='leyendaFiltros text-light'>
            No existen productos para las categorías seleccionadas.
          </p>
        )}
        <BrandSlider />
        <WhatsappButton />
